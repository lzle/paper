...
最简单的一种差错控制形式是在一个字节、字或其他简单数据单元上附加一个单一的奇偶校验位。简单奇偶校验对于检测极少发生的错误是可行的，但当错误概率较高或数据长度较长时，其效果就不再令人满意。
...
不幸的是，单一奇偶校验位只能检测奇数个错误，而无法检测偶数个错误。对于一个 12500 字节（$n = 100{,}000$ 比特）的消息，且比特差错概率为 $p = 10^{-6}$，各概率为：
...
| $P_{\text{odd}}$ | 10.0%   | probability of any odd errors (detected)   |
| $P_{\text{even}}$ | 0.50%   | probability of any even errors (undetected) |
