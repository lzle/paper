...
> 注：新增一个文件时会涉及多个磁盘块的写入，可预测的顺序是先写入数据块，再写入元数据、目录块、位图块等。
...
> 可预测性：同步元数据更新，多块更新必须严格按顺序写入磁盘，后续写入必须等待前序 I/O 完成，代表系统 BSD Fast File System。通过延迟有序写入解决批量写入的性能问题，通过软更新机制解决循环依赖问题。缺点恢复仍需全盘扫描。

> 原子可预测性：Copy-on-Write 原子更新，新版本写入新位置，通过单个“提交点”（如根指针）原子生效，代表：WAFL（以及后来 ZFS / btrfs 的思想）。

> 原子可预测性：日志结构文件系统（LFS），整个文件系统是持续增长的日志结构，提交点天然存在于日志中，恢复只需扫描日志尾部，代表系统：Sprite LFS、Berkeley LFS、Linux LFS。

> 原子可预测性：日志型（Journaling）文件系统，通过日志保障写入是事务性操作，代表系统 ext3、ext4、XFS。
