...
> 注：相比与CPU和内存大的提升，磁盘性能提升有限；文件系统的性能主要受限于磁盘难以改进的机械运动，传输带宽无法显著提升。传输带宽每年增
> 加约 50%～100%。寻道和旋转延迟成本下降得较慢，可能每年 5%～10%。
...
> 注：另一个背景是现有文件系统在许多常见工作负载上表现不佳。例如，FFS 一个文件的写入，至少要进行 6 次磁盘 I/O，因此，尽管 FFS 会将所有这些块放在同一个块组中，但 FFS 会导致许多短寻道和随后的旋转延迟，因此性能远远低于峰值顺序带宽。
...
> 注：对于每一种数据结构，表中给出了该数据结构在 Sprite LFS 中所承担的功能。同时，表中还指出了该数据结构是存储在日志中还是存储在磁盘上的固定位置，并标明了论文中对该数据结构进行详细讨论的具体位置。inode、间接块（indirect blocks）以及超级块（superblocks）与 Unix FFS 中同名的数据结构在概念上是相似的。需要注意的是，Sprite LFS 中既不包含位图（bitmap），也不包含空闲链表（free list）。
...
> 注：LFS 引入了间接层 imap，将 inode 编号作为输入，生成最新版本的 inode 的磁盘地址。移除了 FFS 中的文件位图和 inode 位图。
...
> 注：该示例展示了在创建两个单块文件 dir1/file1 和 dir2/file2 时，Sprite LFS 与 Unix FFS 所写入的被修改磁盘块。两个系统都必须为 file1 和 file2 写入新的数据块和 inode，同时还需要为其所在目录写入新的目录数据块和 inode。Unix FFS 为这些新信息需要执行十次非顺序写入（其中新文件的 inode 各被写入两次，以便在发生崩溃时简化恢复），而 Sprite LFS 则通过一次大的顺序写入完成所有这些操作。两种系统在读取这些文件时所需的磁盘访问次数是相同的。此外，Sprite LFS 还会写出新的 inode 映射块（inode map blocks），以记录新的 inode 位置。
...
> 注：在日志结构化文件系统中，可以通过两种方式为日志生成空闲空间：复制旧块，或使日志在旧块之间穿行。图的左侧展示了“穿行日志”（threaded log）的方法，其中日志跳过仍然有效的块，并覆盖那些已被删除或被覆盖文件的块；同时在日志块之间维护指针，以便在发生崩溃恢复时能够顺序追踪日志。图的右侧展示了“复制方案”，该方案通过读取日志末尾之后的一段磁盘区域，并将其中仍然有效的块与新数据一起重写到新生成的空间中，从而产生新的日志空间。
...
> 注：段摘要块用来记录块所属文件和文件位置，判断块是否存活。通过在磁盘上的段中记录版本号，LFS 可以简单地通过将磁盘版本号与 imap 中的版本号进行比较，跳过上述较长的检查，从而避免额外的读取。
...
> 注：在日志结构化文件系统中，写入成本在很大程度上取决于被清理段的利用率。被清理段中存活数据越多，清理所需的磁盘带宽就越大，从而可用于写入新数据的带宽就越少。该图还给出了两个参考点：“FFS today”，表示当前的 Unix FFS；以及 “FFS improved”，这是我们对改进后的 Unix FFS 所能达到的最佳性能的估计。Unix FFS 的写入成本对磁盘空间使用量并不敏感。
...
> 注：标注为 “FFS today” 和 “FFS improved” 的曲线为便于比较而从图 3 中重绘。“No variance” 曲线表示在所有段始终具有完全相同利用率的情况下所产生的写入成本。“LFS uniform” 曲线表示一种访问模式均匀、并采用贪心清理策略的日志结构化文件系统：清理器选择利用率最低的段。“LFS hot-and-cold” 曲线表示一种具有文件访问局部性的日志结构化文件系统；它同样采用贪心清理策略，并在重新写出之前按年龄对存活数据进行排序。横轴表示整体磁盘容量利用率，这并不一定等同于被清理段的利用率。
...
> 注：该图比较了在冷热（hot-and-cold）访问模式下，贪心策略与成本—收益（cost-benefit）策略的写入成本。结果表明，成本—收益策略明显优于贪心策略，尤其是在磁盘容量利用率高于 60% 时。
...
> 注：图（a）衡量了一个基准测试：创建 10 000 个一千字节的文件，然后按照创建顺序将其读回，最后将其删除。速度以两个文件系统在各个操作阶段每秒处理的文件数来衡量。Sprite LFS 中的日志化方法在文件创建和删除阶段提供了一个数量级的性能提升。图（b）估计了在使用相同磁盘但计算机处理器更快的情况下，各系统在文件创建方面的性能。在 SunOS 中，图（a）所示测试期间磁盘利用率达到 85%，因此更快的处理器几乎不能显著提升性能；而在 Sprite LFS 中，图（a）期间磁盘仅有 17% 被占用，而 CPU 利用率达到 100%，因此 I/O 性能将随着 CPU 速度的提升而线性扩展。
...
> 注：该图展示了一个基准测试的速度：首先通过顺序写入创建一个 100 MB 的文件，然后对该文件进行顺序读取；接着在已有文件上随机写入 100 MB 数据，再从该文件中随机读取 100 MB 数据，最后再次对该文件进行顺序读取。图中分别给出了这五个阶段各自的带宽。除“对先前随机写入的文件进行顺序读取”这一情况外，Sprite LFS 的写入带宽高于 SunOS，而读取带宽与 SunOS 基本相同。
...
> 注：对于每个 Sprite LFS 文件系统，该表列出了磁盘大小、平均文件大小、平均每日写入流量、平均磁盘容量利用率、四个月期间清理的段总数、清理时为空的段所占比例、已清理的非空段的平均利用率，以及测量期间的整体写入成本。这些写入成本数据表明，清理开销将长期写入性能限制在最大顺序写入带宽的约 70%。
...
> [日志结构文件系统](https://pages.cs.wisc.edu/~remzi/OSTEP/Chinese/43.pdf)
