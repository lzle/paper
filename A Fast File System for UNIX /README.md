...
> 柱面组组织，将磁盘划分为柱面组，inode 与数据块尽量放在同一柱面组内，提高物理局部性，减少寻道和旋转延迟。

> 块与碎片管理，使用大块提高顺序访问吞吐量，碎片机制解决小文件存储的空间浪费。

> 目录访问优化，柱面组内 inode 集中布局，使访问目录中所有文件的 inode 所需磁盘传输次数有限（8–16 次），与文件数量无关。
